...
//        }
        ...
        for (int nbLines = readInt(reader); nbLines != 0; nbLines = readInt(reader)) {
//        reader.readLine();
            long[] values = new long[nbLines];
            long total = 0;
            for (int i = 0; i < nbLines; i++) {
                String line = reader.readLine();
                String parts[] = line.split("\\.");
                values[i] = Integer.parseInt(parts[0]) * 100 + Integer.parseInt(parts[1]);
                total += values[i];
            }
            long minMean = total / nbLines;
            long maxMean = minMean;
            long maxSum = 0;
            long minSum = 0;
            if (total % nbLines != 0) {
                maxMean = minMean + 1;
            ...
            for (int i = 0; i < nbLines; i++) {
                if (values[i] > maxMean) {
                    maxSum += values[i] - maxMean;
                } else {
                    minSum += minMean - values[i];
                }
            }
            long r = Math.max(minSum, maxSum);
            formatter.format("$%d.%02d\n", r / 100, r % 100);
        }
