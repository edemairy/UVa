...
//				result.append('\n');
	...
	private int MAX = 500;
	private BigInteger[] vals = new BigInteger[MAX + 1];
...
//        }
		...
		for (int i = 2; i <= MAX; i++) {
			BigInteger o = BigInteger.valueOf(i).pow(i);
			vals[i] = vals[i - 1].multiply(o);
		}
		...
		for (int i = 0; i < Q; i++) {
			...
			res = vals[N].divide(vals[r]).divide(vals[N - r]);
			formatter.format(res.mod(BigInteger.valueOf(M)).toString());
			formatter.format("\n");
		}
